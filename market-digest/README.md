...
你不是群管理员，所以不能把 Bot 加进博主的服务器。用你自己的账号自动抓消息（self-bot）违反 Discord 条款，会被封号，这个程序不做。可以用下面几种方式，也可以同时用。

**方式 ⓪（推荐）：读取 Windows 电脑上的 Discord 通知（全自动，零额外设备）**

程序和 Discord 电脑版在同一台 Windows 电脑上时，程序可以直接读取 Windows 右下角弹出的 Discord 通知。它只读 Windows 已经显示给你的通知，不碰 Discord 本身。

1. Windows 电脑上登录 Discord 电脑版，保持运行（可以最小化到托盘）。
2. 在 Discord 里对博主发点位的频道：右键频道 → 通知设置 → **所有消息**。再确认 Discord 设置 → 通知 → 「启用桌面通知」是打开的。
3. 在 `config.yaml` 的 `discord:` 下加一项，`match` 填通知里一定会出现的词，一般是服务器名和博主昵称：
   ```yaml
   discord:
     - name: 博主名
       match: ["服务器名", "博主昵称"]
       category: both
   ```
4. 运行 `windows\check.bat`。如果 Windows 询问是否允许读取通知，点「允许」；也可以在 设置 → 隐私和安全性 → 通知 里打开「允许应用访问通知」。

注意：
- Windows 打开「请勿打扰 / 专注助手」时，通知不会弹出，程序也就读不到。
- 你正在看那个频道时，Discord 可能不发通知。平时最小化就行。
- 很长的消息在通知里可能被截断，图片看不到。
