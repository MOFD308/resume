...
| 信息源 | 怎么拿 | 延迟 |
|---|---|---|
| YouTube | 发现新视频 → 下载字幕（自动字幕通常上传后 10–60 分钟出现） | 视频出字幕后几分钟 |
...
| 邮件 | 时间 | 内容 |
|---|---|---|
| 盘前点位 | 周一至五 08:45 | 隔夜新点位、接近现价的点位、总表和宏观观点，开头有 AI 写的综述（共识和分歧） |
| 盘中更新 | 12:30 | 上一封之后的新点位，以及现在正在测试的点位 |
| 收盘复盘 | 16:30 | 当天汇总 |
| 宏观周报 | 周日 18:00 | 一周宏观观点综合 |
| 宏观即时提醒 | 博主一发宏观视频/文章 | 字幕总结、分主题观点和风险判断 |

## 大概费用（每月）

| 项目 | 费用 |
|---|---|
| Claude API（提取点位 + 写简报） | 约 $10–40，取决于博主数量和视频长短 |
| X API（按量付费，$0.005/条推文） | 10 个博主、每人每天 20 条，约 $30；不用则 $0 |
| 服务器 | 家里电脑 $0；VPS 约 $5 |
...
python -m market_digest send premarket      # 立刻发一封盘前邮件
...
限制：很长的消息在通知里可能被截断，图片看不到。程序会自动过滤重复通知；对于 `levels` 类的博主，不含数字的闲聊会被跳过，省 API 费用。
